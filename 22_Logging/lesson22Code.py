...
class BlueBlob(Blob):
    ...
    def __add__(self, other_blob):
        logging.info('Blob add Op {}+{}'.format(
            str(self.color), str(other_blob.color)))
...
def is_touching(b1, b2):
    return np.linalg.norm(np.array([b1.x, b1.y]) - np.array([b2.x, b2.y])) < (
        b1.size + b2.size)
    ...
    for blue_id, blue_blob in blues.copy().items():
        for other_blobs in blues, reds, greens:
            for other_blob_id, other_blob in other_blobs.copy().items():
                logging.debug('check touching {}+{}'.format(
        ...
        try:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    pygame.quit()
                    quit()
            blue_blobs, red_blobs, green_blobs = draw_environment(
